...
|------------|----------------|
| Data in CRM, ERP, billing, and files that do not join | Connect sources; land one warehouse. [Sources](/connectors/sources/) |
| Developers are shipping product, not pipelines | Discover, sync, and model without a specialist team |
| Long integration quotes | Install and run a first pipeline locally or in your VPC. [Install](/getting-started/install) |
| Audit and reporting deadlines | Consolidated tables with lineage in your warehouse |

## First hours

1. Connect a source ([quick start](/getting-started/quickstart)).
2. Discover schemas and sync bronze.
3. Run [`sde model`](/cli/model) and review dbt.
